...
test('三主線各拆易/中/難＝9 個任務，同線門檻與獎勵遞增', () => {
  ...
  const diligentGoals = QUEST_LINES.find((line) => line.key === 'diligent').tiers.map((tier) => tier[1]);
  const endure = QUEST_LINES.find((line) => line.key === 'endure');
  assert.equal(endure.metric, 'todayCorrect', '不輟應以答對題數衡量精熟，不計答錯題');
  assert.equal(endure.tiers.some((tier) => diligentGoals.includes(tier[1])), false, '不輟門檻不可與勤學重疊');
});
...
test('未達標不可領（同線高階仍鎖）', () => {
  const m = meta();
  m.daily.todayCorrect = 8; // endure 易達標、中未達
...
test('claimableCount 只算達標且未領', () => {
  const m = meta();
  m.daily.todayCorrect = 5;  // diligent 易達標，不輟易(8)尚未達標
