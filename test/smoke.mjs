...
// 集市達人排行／曾經持有收藏冊：分頁鈕應存在；預先塞一筆 zz_mkt_ever 紀錄後 reload，
// 切到「曾經持有」分頁應渲染出「售予」字樣（賣掉的寶物在收藏冊留痕）
const mktTabStarsExists = await page.$('#mkt-tab-stars') !== null;
const mktTabEverExists = await page.$('#mkt-tab-ever') !== null;
await page.evaluate(() => {
  localStorage.setItem('zz_mkt_ever', JSON.stringify([
    { gearId: 'langhao', dir: 'sold', peer: '小華', ts: Date.now() },
  ]));
});
await page.reload();
await page.click('#advanced-play > summary');
...
if (!mktTabStarsExists) throw new Error('翰墨集市應有 #mkt-tab-stars 集市達人分頁鈕');
if (!mktTabEverExists) throw new Error('翰墨集市應有 #mkt-tab-ever 曾經持有分頁鈕');
if (!mktEverText.includes('售予')) throw new Error(`曾經持有分頁應渲染「售予」字樣，實際 "${mktEverText}"`);
